...
def get_dashboard_metrics(db: Session = Depends(get_db)):
    """Retrieve high-level consolidated security metrics, risk posture, and breakdown."""
    ...
    top_res = [
        ...
        for r in db.query(CloudResource).order_by(desc(CloudResource.risk_score)).limit(5).all()
    ]
    ...
    return RiskDashboardMetrics(
        ...
        compliance_scores={"CIS_AWS_V3": 68.5, "PCI_DSS_V4": 74.0, "SOC2_TYPE2": 81.2},
        ml_anomaly_count_24h=anomalies_count
    )


@router.get("/audit/logs")
def get_audit_trail(db: Session = Depends(get_db)):
    """Retrieve cryptographically chained tamper-evident audit logs."""
    return db.query(AuditLog).order_by(desc(AuditLog.sequence_number)).limit(50).all()


@router.get("/audit/verify")
def verify_audit_integrity(db: Session = Depends(get_db)):
    """Run real cryptographic verification across the entire audit log hash chain."""
    return verify_audit_chain_integrity(db)


@router.get("/compliance/frameworks")
def list_compliance_frameworks(db: Session = Depends(get_db)):
